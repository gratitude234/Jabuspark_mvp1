...
<template>
  <main class="min-h-dvh flex items-center justify-center px-4 py-10">
    <section class="w-full max-w-6xl grid gap-8 md:grid-cols-2 items-center">
      ...
      <aside class="hidden md:block">
        <div class="max-w-lg">
          ...
          <div class="mt-8 panel p-4">
            <p class="text-sm text-text-2">
              <span class="font-semibold text-text">Tip:</span> Use your school email so your account is easier to recover later.
            </p>
          </div>
        </div>
      </aside>
        ...
        <AppCard>
          ...
          <div class="seg w-full">
            <button
              ...
              :class="mode === 'login' ? 'seg-btn--active' : 'seg-btn--inactive'"
              :disabled="busy"
              @click="setMode('login')"
            >
              Sign in
            ...
            <button
              type="button"
              class="seg-btn flex-1"
              :class="mode === 'register' ? 'seg-btn--active' : 'seg-btn--inactive'"
              :disabled="busy"
          ...
          <form class="mt-6 space-y-4" novalidate @submit.prevent="submit">
            <div v-if="errors.form" class="alert alert-danger" role="alert" aria-live="polite">
              {{ errors.form }}
            </div>
            ...
            <Transition name="fade" mode="out-in">
              <div v-if="mode === 'register'" key="fullName">
                ...
                <p v-if="errors.fullName" :id="`${ids.fullName}-error`" class="error">{{ errors.fullName }}</p>
              </div>
            </Transition>
            ...
            <div>
              ...
              <p v-if="errors.email" :id="`${ids.email}-error`" class="error">{{ errors.email }}</p>
            </div>

            <div>
              <div class="flex items-center justify-between gap-3">
                <label class="label" :for="ids.password">Password</label>
                <RouterLink v-if="mode === 'login'" to="/auth/forgot" class="text-sm text-text-2 hover:text-text">
                  Forgot password?
                </RouterLink>
              </div>
              ...
              <div class="relative mt-2">
                ...
                <button
                  type="button"
                  class="absolute right-2 top-1/2 -translate-y-1/2 rounded-xl2 border border-stroke/60 bg-white/[0.02] px-3 py-1.5 text-xs font-semibold text-text-2 hover:bg-white/[0.06] hover:text-text focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/55"
                  :disabled="busy"
              ...
              <div class="mt-2 flex items-center justify-between">
                ...
                <label v-if="mode === 'login'" class="flex items-center gap-2 text-xs text-text-2 select-none">
                  <input
                    type="checkbox"
              ...
              <p v-if="errors.password" :id="`${ids.password}-error`" class="error">{{ errors.password }}</p>
            </div>
            ...
            <p v-if="mode === 'register'" class="pt-1 text-center text-xs text-text-3">
              By creating an account, you agree to our Terms and Privacy Policy.
            </p>

            <p class="pt-1 text-center text-xs text-text-3">
