...
// inline/action errors (validation, buttons, etc.)
...
watch(facultyId, async (next) => {
  // prevent stale error flashing during new loads
  clearInline()
  ...
  try {
    ...
    // don’t block the page; show inline
...
watch([departmentId, level], async () => {
  ...
  try {
    ...
    inlineError.value = e?.message || 'Failed to load department courses.'
    return
  }
...
async function resetToSaved() {
  ...
  try {
    ...
    // safe fallback: show inline only
    inlineError.value = e?.message || 'Failed to refresh after reset.'
  }
}

// --- Save profile (also saves study settings + courses) ---
async function save() {
  // IMPORTANT: clear old inline errors so they don’t flash during new work
  inlineError.value = ''
  savedOk.value = false

  const n = (fullName.value || '').trim()
  if (!n) return (inlineError.value = 'Full name is required.')
  if (!facultyId.value) return (inlineError.value = 'Choose a faculty to continue.')
  if (!departmentId.value) return (inlineError.value = 'Choose a department to continue.')
  if (!level.value) return (inlineError.value = 'Choose your level to continue.')
  if (baseCourseIds.value.length === 0) return (inlineError.value = 'No default courses found for your department/level yet.')
  ...
  try {
    ...
    inlineError.value = e?.message || 'Failed to save changes.'
  } finally {
    busy.value = false
  }
}
...
async function goRepRequest() {
  ...
  if (!hasStudySetup.value) {
    inlineError.value = 'Set your Full name, Faculty, Department and Level first (then save) before requesting upload access.'
    scrollToId('study-settings')
    return
  }

  if (baseCourseIds.value.length === 0) {
    inlineError.value = 'No department courses found for your selected department/level yet. Change department/level or contact admin.'
    scrollToId('study-settings')
    return
  }
...
function goUploads() {
  ...
  if (uploadsDisabled.value) {
    inlineError.value = 'Uploads are disabled for your account. Contact an admin.'
    return
  }
  router.push('/uploads')
}
    ...
    <template v-else>
      ...
      <AppCard id="profile-top" class="relative overflow-hidden">
        ...
        <div class="relative">
          ...
          <div v-else-if="inlineError" class="alert alert-danger mt-4" role="alert">{{ inlineError }}</div>
          ...
          <div class="grid gap-3 sm:grid-cols-4">
            <div class="tile !p-6">
              ...
              <div class="mt-2 font-semibold truncate leading-snug">{{ fullName || '—' }}</div>
            ...
            <div class="tile !p-6">
              <div class="text-xs text-text-3 leading-relaxed">Faculty</div>
              <div class="mt-2 font-semibold truncate leading-snug">{{ displayFaculty }}</div>
              <div class="mt-3 text-xs text-text-3 leading-relaxed">
                Department: <b class="text-text-2">{{ displayDepartment }}</b>
              </div>
            </div>
            ...
            <div class="tile !p-6">
              <div class="text-xs text-text-3 leading-relaxed">Uploader access</div>
              <div class="mt-2 font-semibold flex items-center gap-2 leading-snug">
                <span class="badge" :title="repState">{{ repStateLabel }}</span>
                <span v-if="repBusy" class="text-xs text-text-3">Refreshing…</span>
              </div>
              <div class="mt-3 text-xs text-text-3 truncate leading-relaxed">{{ repHint }}</div>
            </div>
          </div>
